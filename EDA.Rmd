...
Hierarchical / screened interaction strategy:
Start with top HVGs by SD
Use multinomial LASSO (main effects) to identify small anchor set of strong genes.
Create interaction terms only between anchor genes and all HVGs.
Fit a multinomial LASSO on:
main effects + anchor × gene interactions.
Use LASSO to determine significant interactions
...
Methodology:
To assess whether gene–gene interactions improve subtype discrimination, we extended the HVG-based multinomial LASSO framework to include interaction effects. Using the previously fit main-effects model, we identified a sparse set of anchor genes with strong subtype associations. Interaction terms were then constructed between each anchor gene and all highly variable genes, yielding a structured interaction feature set that avoids the combinatorial burden of modeling all pairwise interactions. A LASSO-penalized multinomial logistic regression model was fit to this expanded design matrix using 3-fold cross-validation, allowing simultaneous selection of informative main and interaction effects. Predictive performance was evaluated on a held-out test set and compared directly to the main-effects model.

Results:
Extending the multinomial classifier to include gene–gene interaction terms yielded identical predictive performance (95.8% test accuracy) compared to the main-effects model. Notably, the interaction-augmented LASSO retained 57 interaction terms and zero main-effect terms, indicating that subtype discrimination was driven entirely by combinatorial gene expression patterns rather than marginal gene effects. This result suggests that epistatic relationships among highly variable genes capture subtype-specific structure that is redundant with, but not superior to, marginal gene effects in terms of classification accuracy.

# Support Vector Machine
Plan:
We will use the top 100 discriminatory genes identified by the Multinomial Lasso model as inputs for a Support Vector Machine (SVM).
We will use a Linear Kernel, which is generally effective for high-dimensional gene expression data where the number of features is large relative to the number of samples.
We will use 3-fold cross-validation to tune the cost parameter ($C$) and evaluate the model on the held-out test set.
...
```
Methodology: To assess if a margin-based classifier could outperform the probabilistic Lasso model, we trained a multiclass Support Vector Machine (SVM) with a linear kernel. We utilized the reduced feature set of the top 100 genes selected by the previous multinomial Lasso step to minimize overfitting and maintain dimensionality reduction. The model was trained using the caret framework with 3-fold cross-validation to optimize the cost parameter ($C$), which controls the penalty for misclassified points. Data was centered and scaled prior to training, as SVMs are distance-based and sensitive to variable magnitudes.

Results: The SVM model selected an optimal cost parameter of $C = 0.1$ (check output). On the independent test set, the SVM achieved an accuracy of `r round(svm_acc * 100, 1)%`, indicating a worse performance relative to the Multinomial Lasso model. The confusion matrix indicates that the model was able to separate distinct tumor types with high precision, but struggled with the glioblastoma class. 
